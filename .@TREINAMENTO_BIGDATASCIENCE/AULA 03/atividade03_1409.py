...
try:
    ...
    # divide a distribuição em duas partes iguais (50% dos dados abaixo e 50%
    # acima)
    mediana_cvli = np.median(array_cvli)

    # distânicia
    distancia = abs((media_cvli-mediana_cvli)/mediana_cvli)

    # Medidas de tendência central
    # Se a média for muito diferente da mediana, distribuição é assimétrica.
    # Não tende a haver um padrão
    # e pode ser que existam outliers (valores discrepantes)
    # Se a média for próxima (25%) a mediana, distribuição é simétrica. Tende
    # a haver um padrão
    print('\nMedidas de tendência central: ')
    print(30*'-')
    print(f'Média de roubo de veículos: {media_cvli}')
    print(f'Mediana de roubo de veículos: {mediana_cvli}')
    ...
    # Quanto masi próximo do máximo, maior a dispersão dos dados 
    # ou heterogeneidade
    ...
    q1 = np.quantile(array_cvli, 0.25, method='weibull')  # Q1 é 25%
    q2 = np.quantile(array_cvli, 0.50, method='weibull')  # Q2 é 50% (mediana)
    q3 = np.quantile(array_cvli, 0.75, method='weibull')  # Q3 é 75%
    ...
    # filtrar o dataframe df_cvli para o munics com roubo de veículo abaixo q1
    df_cvli_outliers_inferiores = df_cvli[df_cvli['cvli'] < limite_inferior]

    # filtrar o dataframe df_cvli para o munics com roubo de veículo acima q3
    df_cvli_outliers_superiores = df_cvli[df_cvli['cvli'] > limite_superior]
    ...
    if len(df_cvli_outliers_inferiores) == 0:
        ...
        print(df_cvli_outliers_inferiores.sort_values(
            by='aisp', ascending=True))
    ...
    if len(df_cvli_outliers_superiores) == 0:
        ...
        print(df_cvli_outliers_superiores.sort_values(
            by='aisp', ascending=False))
...
try:
    ...
    plt.subplot(2, 2, 1)
    plt.boxplot(array_cvli, vert=False, showfliers=False)
    ...
    # ordenar
    df_cvli_outliers_superiores_ordered = df_cvli_outliers_superiores.sort_values(
        by='cvli', ascending=True)

    plt.barh(df_cvli_outliers_superiores_ordered['aisp'],
             df_cvli_outliers_superiores_ordered['cvli'])
    ...
    plt.text(0.1, 0.9, f'Média: {media_cvli}', fontsize=12)
    plt.text(0.1, 0.8, f'Mediana: {mediana_cvli}', fontsize=12)
